...
@channel Immediate attention required.
  ...
  async sendEmergencyNotification(data: {
    ...
    const alert = this.getAlert(emergencyAlert);
    if (!alert) {
      console.error(`Failed to retrieve emergency alert: ${emergencyAlert}`);
      return;
    }
    ...
    for (const channel of criticalChannels) {
      await this.sendToChannel(channel, alert, 'emergency');
    }
