...
export class ReconciliationService {
    ...
    for (const achReturn of returns) {
      try {
        if (achReturn.status === 'failed' && achReturn.failure_code) {
          ...
          const { stripeJournalService } = await import('./stripeJournalService.js');
          ...
          const journalEntry = await (stripeJournalService as any).createACHReturnEntry(returnData);
  ...
  private async getJournalEntriesForPeriod(startDate: string, endDate: string): Promise<JournalEntry[]> {
    ...
    return (allEntries as any[]).filter(entry => {
