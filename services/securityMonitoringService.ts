...
export interface SecurityEvent {
  ...
  type: 'LOGIN_FAILURE' | 'SUSPICIOUS_TRANSACTION' | 'MULTIPLE_FAILED_LOGINS' | 
        ...
        'COMPLIANCE_REPORT_GENERATED' | 'CONTROL_ASSESSMENT_OVERDUE' |
        'INCIDENT_STATUS_CHANGED' | 'INCIDENT_CREATED' | 'SECURITY_CONTROL_UPDATED' |
        'API_ANOMALY_DETECTED' | 'UNAUTHORIZED_ACCESS_ATTEMPT';
...
export interface ThreatDetectionRule {
  ...
  name: string;
  description: string;
  type: 'SIGNATURE' | 'BEHAVIORAL' | 'ANOMALY' | 'COMPLIANCE';
  severity: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL';
  enabled: boolean;
  conditions: any[];
}
...
export class SecurityMonitoringService {
  ...
  async logSecurityEvent(event: {
    ...
    return this.logEvent({
      ...
      severity: event.severity.toUpperCase() as SecurityEvent['severity'],
  ...
  async getSecurityMetrics(timeRange?: string): Promise<SecurityMetrics> {
    ...
    return {
      ...
      openIncidents: metrics.activeIncidents,
      resolvedIncidents: this.incidents.filter(i => i.status === 'RESOLVED').length,
      criticalEvents: metrics.criticalEvents,
      avgResolutionTimeMs: metrics.averageResponseTime
    };
  }

  async getSecurityStatus(): Promise<{ recentEvents: any[] }> {
    return {
