...
const { testEnvironmentSetup, getStripe, getTestCustomer, getTestUser, setupTestEnvironment } = require('./test-environment-setup.cjs');
...
class TestStripeApis {
  ...
  async initialize() {
    ...
    // Initialize shared environment if not already down
    if (!this.stripe) {
       this.stripe = getStripe();
    }
    
    this.apiUrl = process.env.TEST_API_URL || testEnvironmentSetup.config.application.apiUrl || 'http://localhost:3005/api/stripe';
    
    // Setup authentication headers with PROVIDED CREDENTIALS
    this.authHeaders = {
        'Authorization': `Bearer mock_admin_token`,
        'Content-Type': 'application/json',
        'X-User-ID': 'user_admin_live_001',
        'X-User-Email': 'ADMIN@SOVR.CREDIT'
    };
    
    console.log('✅ Stripe API Integration Tests initialized with ADMIN@SOVR.CREDIT');
  }
  ...
  async runAllTests() {
    ...
    const testMethods = [
      ...
      this.testCustomerApi,
      // this.testPaymentMethodApi, // TODO: Update to match nested resource /customers/:id/payment-methods
      // this.testPaymentIntentApi, // TODO: Update to match /ach/payment-intents
      // this.testChargesApi,       // Not implemented
      // this.testBalanceApi,       // Not implemented
      // this.testWebhookApi,       // Not implemented (management)
      // this.testErrorHandling,    // Needs updates for schema
      // this.testRateLimiting,     // Not implemented
      this.testAuthentication,
      // this.testAuthorization,    // Needs updates for schema
      this.testApiVersioning,
      // this.testIdempotencyKeys, // Needs updates for schema
      // this.testRequestResponseValidation, // Needs updates for schema
      // this.testRetryMechanism,   // Needs updates
      // this.testConcurrency       // Needs updates
    ];
  ...
  async testCustomerApi() {
    ...
    try {
      ...
      const updateCustomerResponse = await this.makeApiCall('PUT', `/customers/${createCustomerResponse.data.id}`, {
        ...
        lastName: 'Test Customer',
        stripeMetadata: { test_api: 'true', updated: 'true' }
      });
      assert.equal(updateCustomerResponse.status, 200, 'Customer should be updated');
      assert.equal(updateCustomerResponse.data.firstName, 'Updated API',
  ...
  async testAuthorization() {
    ...
    try {
      ...
      const accountantAuthHeaders = {
        ...
        'X-User-Email': accountantUser.email
      };
...
module.exports = {
  ...
  runStripeApiTests: async () => {
    // Initialize environment first to get Stripe instance
