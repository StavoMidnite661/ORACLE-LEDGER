...
// Local UI-specific metrics (can be extended or mapped)
...
const SecurityMetrics: React.FC = () => {
  ...
  const loadMetricsData = async () => {
    try {
      ...
      calculateKPIScores([securityData] as any, alertData || undefined, [complianceData] as any);
  ...
  const calculateKPIScores = (
    ...
    const latestSecurity = securityData[securityData.length -1] as any;
    const latestCompliance = complianceData[complianceData.length -1];
    
    const kpis: KPIScore[] = [
      {
        name: 'System Health',
        current: (latestSecurity as any).systemHealth || 0,
        target: 95,
        trend: 2.5,
        status: ((latestSecurity as any).systemHealth || 0) >= 95 ? 'good' : 
              ((latestSecurity as any).systemHealth || 0) >= 80 ? 'warning' : 'critical',
